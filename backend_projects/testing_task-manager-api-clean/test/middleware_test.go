...
func (suite *MiddlewareTestSuite) TestAuthMiddleware_MissingUserID() {
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, jwt.MapClaims{
		"username": "testuser",
		"role":     "admin",
	})
...
func (suite *MiddlewareTestSuite) TestAuthMiddleware_MissingUsername() {
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, jwt.MapClaims{
		"user_id": "123",
		"role":    "admin",
	})
	...
	assert.Equal(suite.T(), http.StatusUnauthorized, w.Code)
	assert.Contains(suite.T(), w.Body.String(), "Username not found in token")
}

func (suite *MiddlewareTestSuite) TestAuthMiddleware_MissingRole() {
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, jwt.MapClaims{
		"user_id":  "123",
		"username": "testuser",
	})
	...
	assert.Equal(suite.T(), http.StatusUnauthorized, w.Code)
	assert.Contains(suite.T(), w.Body.String(), "role not found in token")
}
